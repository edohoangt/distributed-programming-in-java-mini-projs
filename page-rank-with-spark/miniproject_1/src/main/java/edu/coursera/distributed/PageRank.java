...
public final class PageRank {
    ...
    public static JavaPairRDD<Integer, Double> sparkPageRank(
            ...
            JavaPairRDD<Integer, Double> newRanks = sites.join(ranks)
                    .flatMapToPair(kv -> {
                        Website curSite = kv._2()._1();
                        Double curSiteRank = kv._2()._2();

                        List<Tuple2<Integer, Double>> contribs =
                                new LinkedList<>();
                        Iterator<Integer> outSites = curSite.edgeIterator();
                        while (outSites.hasNext()) {
                            final int next = outSites.next();
                            contribs.add(new Tuple2<>(next, curSiteRank / (double) curSite.getNEdges()));
                        }
                        return contribs;
                    });
